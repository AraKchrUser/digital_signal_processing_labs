...
def load_vad_markup(path_to_rttm, signal, fs):
    ...
    start_col = 3
    duration_col = 4
    with open(path_to_rttm) as frttm:
        for item in map(lambda x: x.split(), frttm.readlines()):
            start = int(float(item[start_col]) * fs)
            duration = int(float(item[duration_col]) * fs)
            vad_markup[start: duration + start] = 1
...
def gmm_train(E, gauss_pdf, n_realignment):
    ...
    for n in range(n_realignment):
        ...
        for j in range(len(w)):
            ...
            sigma[j] = np.sqrt(np.sum(g[:, j] * np.power(E - m[j], 2)) / (M * w[j]))
...
def awgn(signal, sigma_noise):
    ...
    signal_noise = signal + np.random.normal(scale=sigma_noise, size=len(signal))
